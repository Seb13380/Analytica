...
$rows = Transaction::query()
    ...
    ->orderBy('id')
    ->get(['id', 'bank_account_id', 'date', 'amount', 'type', 'normalized_label', 'label']);
...
foreach ($rows as $tx) {
    ...
    $targetDate = $bestDate->format('Y-m-d');
    $oldDate = Carbon::parse((string) $tx->date)->format('Y-m-d');
    ...
    echo sprintf("Deleted duplicate mismatch #%d: %s (target %s already exists)\n", $tx->id, $oldDate, $targetDate);
}
